    ...
    watchConfigSyncStatus(callback) {
        ...
        return this.endpoint.get('/mgmt/tm/cm/sync-status')
            .then((resp) => {
                ...
                if (resp.data.entries['https://localhost/mgmt/tm/cm/sync-status/0'].nestedStats.entries.status.description === 'Standalone') {
            ...
            .then(() => {
                ...
                const getTimeSinceLastSync = () => {
                    ...
                    this.endpoint.get('/mgmt/tm/cm/device-group')
                        .then((response) => {
                            ...
                            return response.data.items.filter(item => item.partition === 'Common' && !(defaultDeviceGroups.includes(item.name)));
                        })
                        .then((deviceGroups) => {
                            deviceGroups.forEach((dg) => {
                                // get statistics for each device group that might sync our config
                                this.endpoint.get(`/mgmt/tm/cm/device-group/${dg.name}/stats`)
                                    .then((dgStats) => {
                                        // loop through devices in device group to find this device
                                        Object.values(dgStats.data.entries).forEach((stats) => {
                                            if (stats.nestedStats.entries.timeSinceLastSync.description <= pollInterval
                                                && stats.nestedStats.entries.device.description === hostname
                                            ) {
                                                callback();
                                            }
                                        });
                                        ...
                                        confSyncTimeout = setTimeout(() => { getTimeSinceLastSync(); }, pollInterval);
                                    })
                ...
                // if deviceInfo is yet undefined - which can happen in first fastWorker.onStart - recurse
