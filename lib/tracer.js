...
class Tracer {
    ...
    close() {
        if (this._tracer) {
            try {
                if (this._debug) {
                    this._logger.fine('Closing tracer.');
                }
                this._tracer.close();
            } catch (e) {
...
class Util {
     ...
     *
     * @param {Object} [sysVersionData]     - the JSON object response from /tm/sys/info
     *
     * @returns {Object} tracer span
     */
    static buildDeviceTags(sysVersionData) {
        const entries = sysVersionData.entries[Object.keys(sysVersionData.entries)[0]].nestedStats.entries;
        return {
            [Tags.DEVICE.PLATFORM]: entries.Product.description,
            [Tags.DEVICE.VERSION]: entries.Version.description,
            [Tags.DEVICE.BUILD]: entries.Build.description,
            [Tags.DEVICE.EDITION]: entries.Edition.description
        };
    }
}
