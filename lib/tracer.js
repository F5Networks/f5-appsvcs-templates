...
class Tracer {
    constructor(serviceName, options) {
        ...
        options = options || {};
        this.enabled = options.enabled || (String(process.env.F5_PERF_TRACING_ENABLED).toLowerCase() === 'true');
        // read-only
        Object.defineProperties(this, {
            serviceName: { value: serviceName },
            _endpoint: { value: options.endpoint || process.env.F5_PERF_TRACING_ENDPOINT },
            _debug: { value: options.debug || String(process.env.F5_PERF_TRACING_DEBUG).toLowerCase() === 'true' }
        });
    ...
    _initTracer(options) {
        ...
        try {
            ...
            if (this._debug) {
                this._logger.fine(`Initialized tracer. enabled: ${this.enabled} api: ${this._endpoint}`);
            }
        } catch (err) {
