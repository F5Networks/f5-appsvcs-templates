...
const TRACER_DEBUG = String(process.env.F5_PERF_TRACING_DEBUG).toLowerCase() === 'true';
...
class Tracer {
    constructor(serviceName, options) {
        ...
        options = options || {};
        if (TRACER_DEBUG) {
            // provide default console when no logger present
        ...
        Object.defineProperties(this, {
            enabled: { value: (String(process.env.F5_PERF_TRACING_ENABLED).toLowerCase() === 'true') },
            serviceName: { value: serviceName },
            logger: { value: options.logger }
        });
    ...
    startSpan(operation, options) {
        ...
        try {
            ...
            return new Span(span, this.logger);
        } catch (e) {
    ...
    _initTracer(options) {
        ...
        if (this.enabled) {
            try {
                if (this.logger) {
                    this.logger.fine('Initializing Jaeger Client');
                }
                ...
                process.env.JAEGER_DISABLED = !(String(process.env.F5_PERF_TRACING_ENABLED).toLowerCase() === 'true');
                process.env.JAEGER_ENDPOINT = process.env.F5_PERF_TRACING_ENDPOINT;
                tracer = JaegerClient.initTracerFromEnv(tracerConf, options);
            } catch (err) {
                this._handleException('Failed to initialize Jaeger Client tracer', err);
            }
        }
        return tracer;
    }
