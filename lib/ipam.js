...
/** Class representing an IPAM Provider. */
class IpamProviders {
    constructor(options) {
        options = options || {};
    ...
    /**
     * create IPAM Schema
     * @param {string} service - name of IPAM Provider type
     * @param {Object} overrides - customized schema
     * @returns {Object}
     */
    ...
    /**
     * create Auth config for IPAM Provider
     * @param {string} provider - name of IPAM Provider
     * @returns {Promise}
     */
    _generateAuthConfig(provider) {
        ...
        if (useCustomAuthHeader) {
            if (provider.authHeaderValue.indexOf('$M$') === 0 || provider.authHeaderValue.indexOf('VG9r') === 0) {
                return Promise.resolve()
                    .then(() => this.secretsManager.decrypt(provider.authHeaderValue))
                    .then(providerAuthHeaderValue => ({
                        headers: {
                            [provider.authHeaderName]: providerAuthHeaderValue
                        }
                    }));
            }
            return Promise.resolve({
    ...
    /**
     * get Address from IPAM and plug it into template properties
     * @param {Object[]} ipFromIpamProps - array of Properties needing addresses from IPAM
     * @param {Object} templateData - template being updated
     * @param {Object} config - FAST Global Settings configuration
     * @param {number} reqid - FASTWorker process id, identifying the request
     * @param {Object} ipamAddrs - IPAM Address objects
     * @returns {Promise}
     */
                ...
                if (provider) {
                    ...
                    ipamChain = ipamChain
                        ...
                        .then(authConfig => this.recordTransaction(
                            reqid,
                            `fetching address from IPAM provider: ${providerName}`,
                            axios.request(merge({
    ...
    /**
     * give unused IPAM Address back to Provider
     * @param {number} reqid - FASTWorker process id, identifying the request
     * @param {Object} config - FAST Global Settings configuration
     * @param {Object} appData - IPAM Address objects
     * @param {Object[]} excludeAddrs - IPAM Addresses to keep
     * @returns {Promise}
     */
            ...
            addrData.forEach((data) => {
                ...
                if (!excludeAddrs || !excludeAddrs[provider.name]
                    ...
                    promises.push(Promise.resolve()
                        ...
                        .then(authConfig => this.recordTransaction(
                            reqid,
                            `releasing ${address} from IPAM provider: ${providerName}`,
                            axios.request(merge({
