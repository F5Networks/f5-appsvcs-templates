...
class AS3Driver {
    constructor(options) {
        options = options || {};
        if (options.span) {
            options.span.log('creating_as3_driver');
        }
        ...
        setTimeout((() => this._handlePendingTasks(options.span)), this._pendingTaskPollRate);
    }
    ...
    _handlePendingTasks(span) {
        return Promise.resolve()
            .then(() => (this._pendingTasks.length === 0 ? Promise.resolve() : this.getTasks(span)))
            ...
            .finally(() => Promise.resolve()
                .then(() => this._releaseOpLock(span))
                .then(() => setTimeout((() => this._handlePendingTasks(span)), this._pendingTaskPollRate)));
    }
    ...
    listApplicationNames(ctx) {
        let span;
        if (ctx.span) {
            span = this._buildAndStartSpan('listApplicationNames', ctx);
        } else {
            span = ctx;
        }
