...
const { BigipDeviceClassic } = require('./bigipDevices');
...
class AS3Driver {
    constructor(options) {
        ...
        this._bigip = options.bigipDevice || new BigipDeviceClassic(options.bigipInfo);
    ...
    loadMixins(ctx) {
        return Promise.resolve()
            .then(() => {
                return Promise.all([
                    this._bigip.getSharedObjectsName('ltm/node', { address: '255.255.255.254' }, ctx),
                    this._bigip.getSharedObjectsName('ltm/virtual-address', { address: '255.255.255.254' }, ctx)
                ])
            })
            .then(([foundNodes, foundVirtualAddresses]) => {
                let tmplData = fs.readFileSync(path.join(__dirname, 'tscommon.yaml'), 'utf8');
                if (foundNodes.length > 0){
                    tmplData = tmplData.replaceAll('fast_telemetry_node', foundNodes[0]);
                }
                if (foundVirtualAddresses.length > 0){
                    tmplData = tmplData.replaceAll('fast_svc_vip', foundVirtualAddresses[0]);
                }
