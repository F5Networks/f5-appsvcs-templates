...
class AS3Driver {
    ...
    /**
     * update TS/ASM/AFM settings with current provision data
     * @param {Object} settings - FAST Global Settings configuration
     * @param {Object} provisionData - Provision data for FAST Global Settings
     * @returns {Object}
     */
     ...
     * @param {Object} settings - FAST Global Settings configuration
     * @param {Object} provisionData - Provision data for FAST Global Settings
     * @param {boolean} skipPost - if set, don't POST these settings to AS3
     * @param {Object} options - optional extra context
     * @returns {Promise}
     */
    setSettings(settings, provisionData, skipPost, options) {
        ...
        return Promise.resolve()
            ...
            .then(decl => this._postDecl(decl, 'Common', options));
    }
    ...
    _postDecl(decl, tenants, options) {
        ...
        const postUrl = `/declare${tenants}?async=true`;
        ...
        return Promise.resolve()
            ...
            .then(() => this._tracePromise(
                options,
                `POST to ${postUrl}`,
                this._endpoint.post(postUrl, decl)
            ))
            .then((result) => {
