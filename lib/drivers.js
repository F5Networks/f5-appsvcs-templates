...
class AS3Driver {
    ...
    calculateTsSettings(provisionData) {
    ...
    setSettings(settings, provisionData, skipPost) {
        settings = this.calculateTsSettings(provisionData[0]);
        settings.ipamProviders = this._setIpamDefaults(settings);
    ...
    getTasks() {
        ...
        const as3ToFAST = (item) => {
            ...
            if (declid) {
                ...
                if (results.length > 0) {
                    host = results[0].host || 'localhost';
                }
                results = results.filter(r => r.tenant === tenant);
                if (results.length === 0) {
            ...
            return {
                id: item.id,
                code: (item.results && item.results[0]) ? item.results[0].code : '',
