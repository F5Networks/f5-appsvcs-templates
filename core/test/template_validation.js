...
const { FsTemplateProvider, GitHubTemplateProvider } = require('../lib/template_provider.js');

const provider = new FsTemplateProvider('./../templates', './../schemas');
...
const outfile = {
    write: (message) => {
        const param = (() => {
            if (Buffer.isBuffer(message)) return `${new Date()} ${message.toString('utf8')}\n`;
            if (typeof message === 'string') return `${new Date()} ${message}\n`;
...
const runTemplate = async (templateName, context) => {
    const engine = await provider.fetch(templateName);
...
describe('template valid-json', () => {
    it('audit template tests', () => {
        files.map(i => i.split('.')[0])
            .forEach((f) => {
                if (f && !inputMap[f]) throw new Error(`No test for template ${f}`);
            });
    });

    it('provider FsTemplateProvider list', async () => {
        outfile.write('FsTemplatePrivder.list');
        const result = await provider.list();
        outfile.write(result);
    });

    it('provider GitHubTemplateProvider list', async () => {
        outfile.write('GitHubTemplateProvider.list');
        const githubProvider = new GitHubTemplateProvider('zinkem5/f5-as3-templates');
        const result = await githubProvider.list();
        outfile.write(result);
    });

    Object.keys(inputMap).forEach((i) => {
        it(i, async function f() {
            outfile.write(this.title);
            outfile.write(i);
            const result = await runTemplate(i, inputMap[i]);
            outfile.write(inputMap[i]);
            outfile.write(result);
            outfile.write(result.class);
        });
    });

    it('simple_https_2', async function f() {
        const context = {
        ...
        const result = await runTemplate('simple_https', context);
        outfile.write(this.title);
    ...
    it('simple_https_3', async function f() {
        const context = {
    ...
    it('simple_http_1', async function f() {
        const context = {
            uuid: '3456',
            tenant_name: 'zinke',
            application_name: 'app2',
        ...
        const result = await runTemplate('simple_http', context);
        outfile.write(this.title);
        outfile.write(context);
        outfile.write(result);
        outfile.write(result.class);
    });
    it('simple_http_2', async function f() {
        const context = {
            uuid: '3456',
            tenant_name: 'zinke',
            application_name: 'app2',
    ...
    it('cdt_service 1', async () => {
        ...
        const result = await runTemplate('f5_service', context);
        outfile.write(context);
        outfile.write(result);
        outfile.write(result.class);
    });

    it('cdt_service defined app name', async () => {
        /**
        ...
        if (!result.declaration.zinke.arbitrary42
            || Object.keys(result.declaration.zinke).length !== 2) throw new Error('error in application_name');
    });
});
